...
const express = require("express");const cors = require("cors");const path = require("path");const crypto = require("crypto");const rateLimit = require("express-rate-limit");const Stripe = require("stripe");const { OpenAI } = require("openai");
...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY);const ADMIN_SECRET = process.env.ADMIN_SECRET;

const FRONTEND_URL = "https://kiri-frontend.vercel.app";

// STRIPE WEBHOOK MUST BE BEFORE express.json()app.post("/webhook", express.raw({ type: "application/json" }), async (req, res) => {const sig = req.headers["stripe-signature"];
...
} catch (err) {console.log("❌ Webhook error:", err.message);res.sendStatus(400);}});

// MIDDLEWAREapp.use(cors({origin: FRONTEND_URL,methods: ["GET", "POST", "OPTIONS"],allowedHeaders: ["Content-Type", "x-admin-secret"]}));

app.use(express.json());app.use(express.static(__dirname));

const askLimiter = rateLimit({windowMs: 60 * 1000,max: 30,standardHeaders: true,legacyHeaders: false});

// HELPERSfunction verifyAdmin(req, res, next) {const adminKey = req.headers["x-admin-secret"];

if (!ADMIN_SECRET || adminKey !== ADMIN_SECRET) {return res.status(403).json({ error: "Unauthorized" });}

next();}

async function getSiteByKey(site_key) {const result = await pool.query(    SELECT 
      s.*,
      p.name AS plan_name,
      p.monthly_limit
    FROM sites s
...
async function checkConversationAccess(conversationId, site_key) {const result = await pool.query(    SELECT c.id
...
// ROOTapp.get("/", (req, res) => {res.sendFile(path.join(__dirname, "chat.html"));});

// ASK / WIDGET CHATapp.post("/ask", askLimiter, async (req, res) => {try {const { message, conversationId, site_key, language } = req.body;
...
Website knowledge:${knowledgeText}
...
const response = await client.responses.create({model: "gpt-4.1-mini",input: `${systemPrompt}

Conversation so far:${conversationText}

Assistant:`,temperature: 0.4,max_output_tokens: 300});
...
} catch (err) {console.error("ASK ERROR:", err);res.status(500).json({ error: "Internal server error" });}});

// AUTHapp.post("/register", async (req, res) => {try {const { email, password } = req.body;
...
} catch (err) {console.error("REGISTER ERROR:", err);res.status(500).json({ error: "Register failed" });}});
...
// CLIENT DATAapp.post("/site-usage", async (req, res) => {try {const { siteKey } = req.body;
...
} catch (err) {console.error("SITE USAGE ERROR:", err);res.status(500).json({ error: "Server error" });}});

app.get("/conversations", async (req, res) => {try {const { site_key } = req.query;
...
app.get("/messages/", async (req, res) => {try {const { conversationId } = req.params;const { site_key } = req.query;
...
app.post("/reply", async (req, res) => {try {const { conversationId, message, site_key } = req.body;
...
} catch (err) {console.error("REPLY ERROR:", err);res.status(500).json({ error: "Server error" });}});

app.post("/return-to-ai", async (req, res) => {try {const { conversationId, site_key } = req.body;
...
} catch (err) {console.error("RETURN TO AI ERROR:", err);res.status(500).json({ error: "Server error" });}});

// WIDGET SETTINGSapp.get("/widget-settings", async (req, res) => {try {const { site_key } = req.query;
...
} catch (err) {console.error("WIDGET SETTINGS ERROR:", err);res.status(500).json({ error: "Server error" });}});

app.post("/widget-settings", async (req, res) => {try {const {site_key,widget_title,welcome_message,primary_color,bot_instructions} = req.body;
...
} catch (err) {console.error("SAVE WIDGET SETTINGS ERROR:", err);res.status(500).json({ error: "Server error" });}});

// STRIPEapp.post("/create-checkout", async (req, res) => {try {const { site_key } = req.body;
...
// ADMIN ROUTESapp.get("/admin-overview", verifyAdmin, async (req, res) => {try {const result = await pool.query(      SELECT 
        s.id,
        s.name,
        s.site_key,
        s.monthly_message_count,
...
} catch (err) {console.error("ADMIN OVERVIEW ERROR:", err);res.status(500).json({ error: "Server error" });}});

app.post("/admin-update-plan", verifyAdmin, async (req, res) => {try {const { siteId, newPlanName } = req.body;
...
} catch (err) {console.error("ADMIN UPDATE PLAN ERROR:", err);res.status(500).json({ error: "Server error" });}});

app.get("/admin/conversations", verifyAdmin, async (req, res) => {try {const result = await pool.query(      SELECT 
        c.id,
        c.created_at,
        c.human_takeover,
        s.name AS site_name,
...
} catch (err) {console.error("ADMIN CONVERSATIONS ERROR:", err);res.status(500).json({ error: "Server error" });}});

app.get("/admin/messages/", verifyAdmin, async (req, res) => {try {const { conversationId } = req.params;
...
} catch (err) {console.error("ADMIN MESSAGES ERROR:", err);res.status(500).json({ error: "Server error" });}});

// KNOWLEDGE SCRAPE ADMINapp.post("/admin-scrape-site", verifyAdmin, async (req, res) => {try {const { siteId, url } = req.body;
...
} catch (err) {console.error("SCRAPE ERROR:", err);res.status(500).json({ error: "Scrape failed" });}});

// START SERVERconst PORT = process.env.PORT || 3000;

app.listen(PORT, "0.0.0.0", () => {console.log("Kiri backend running on port " + PORT);});
